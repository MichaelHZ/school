...
if ($rec == 'default') {
    ...
    $sql = "SELECT id, title, cat_id, image, add_time,blog_id FROM " . $dou->table('article') . $where . " ORDER BY id DESC" . $limit;
//     echo $sql;
//     exit;
    $query = $dou->query($sql);
...
elseif ($rec == 'insert') {
	...
	if(!empty($_POST['file2'])){
	  $file2 = trim(str_replace('http://'.$_SERVER['HTTP_HOST'],'',$_POST['file2']),'/');
   }
	 // 判断是否有上传图片/上传图片生成
    ...
    if(!empty($_POST['file3'])){
	  $file3 = trim(str_replace('http://'.$_SERVER['HTTP_HOST'],'',$_POST['file3']),'/');
   }
    // 格式化自定义参数
...
elseif ($rec == 'update') {
    ...
    if ($_FILES['image2']['name'] != "") {
	 ...
	 if(empty($up_file2) && !empty($_POST['file2'])){
	  $up_file2 =  ", image2='".trim(str_replace('http://'.$_SERVER['HTTP_HOST'],'',$_POST['file2']),'/')."'";
    }
	 // 上传图片生成
    if ($_FILES['image3']['name'] != "") {
        // 获取图片文件名
	 ...
	 if(empty($up_file3) && !empty($_POST['file3'])){
	  $up_file3 =  ", image3='".trim(str_replace('http://'.$_SERVER['HTTP_HOST'],'',$_POST['file3']),'/')."'";
   }
    // 格式化自定义参数
    $_POST['defined'] = str_replace("\r\n", ',', $_POST['defined']);
    ...
    $add_time = !empty($_POST['add_time'])? strtotime($_POST['add_time']) : time();
	$_POST['password'] =  $_POST['password'] ? $_POST['password'] : 0;
    $sql = "UPDATE " . $dou->table('article') . " SET title = '$_POST[title]', defined = '$_POST[defined]' ,content = '$_POST[content]'" . $up_file . $up_file2 . $up_file3 .", keywords = '$_POST[keywords]', description = '$_POST[description]',custom_url = '$_POST[custom_url]',add_time='$add_time',blog_id='$_POST[cat_id]',password='$_POST[password]' WHERE id = '$_POST[id]' ";
   
	$dou->query($sql);
    
    $dou->create_admin_log('修改博文' . ': ' . $_POST['title']);
    $dou->dou_msg('修改博文成功', 'blog.php');
} 
...
elseif ($rec == 'set_sort') {
    // 验证并获取合法的ID
    $id = $check->is_number($_REQUEST['id']) ? $_REQUEST['id'] : $dou->dou_msg($_LANG['illegal'], 'blog.php');
...
elseif ($rec == 'del') {
    ...
    if (isset($_POST['confirm']) ? $_POST['confirm'] : '') {
        ...
        $dou->delete($dou->table('article'), "id = $id", 'blog.php');
    } else {
        $_LANG['del_check'] = preg_replace('/d%/Ums', $title, $_LANG['del_check']);
        $dou->dou_msg($_LANG['del_check'], 'blog.php', '', '30', "blog.php?rec=del&id=$id");
    }
} 
...
elseif ($rec == 'action') {
    if (is_array($_POST['checkbox'])) {
        if ($_POST['action'] == 'del_all') {
            // 批量文章删除
            $dou->del_all('article', $_POST['checkbox'], 'id', 'image','blog.php');
        } elseif ($_POST['action'] == 'category_move') {
            // 批量移动分类
            $dou->category_move('article', $_POST['checkbox'], $_POST['new_cat_id']);
        } else {
            $dou->dou_msg($_LANG['select_empty'],'blog.php');
        }
    } else {
        $dou->dou_msg($_LANG['article_select_empty'],'blog.php');
    }
}
