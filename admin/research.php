...
if ($rec == 'default') {
    ...
    $smarty->assign('ur_here', '课题研究文章');
    $href = isset($_GET['cat_id']) && intval($_GET['cat_id']) > 0 ? 'research.php?rec=add&cat_id='.intval($_GET['cat_id']):'special.php?rec=add';
    ...
    // 筛选条件
    $where = ' WHERE cat_id IN (' . $cat_id . $dou->dou_child_id('research_category', $cat_id) . ') And stype = 1 ';
    if ($keyword) {
    ...
    $page_url = 'special_page.php' . ($cat_id ? '?cat_id=' . $cat_id : '');
    $limit = $dou->pager('research', 15, $page, $page_url, $where, $get);
    
    $sql = "SELECT id, title, cat_id, image, add_time,user_id,is_password FROM " . $dou->table('research') . $where . " ORDER BY id DESC" . $limit;
    $query = $dou->query($sql);
    while ($row = $dou->fetch_array($query)) {
        $cat_name = $dou->get_one("SELECT cat_name FROM " . $dou->table('research_category') . " WHERE cat_id = '$row[cat_id]'");
 ...
 */
elseif ($rec == 'add') {
    $smarty->assign('ur_here', '课题研究文章添加');
	$href = $_GET['cat_id']?'special_page.php?cat_id='.$_GET['cat_id']:'special_page.php';
    $smarty->assign('action_link', array (
            'text' => '课题研究文章添加',
            'href' => $href, 
    ));
    ...
    // CSRF防御令牌生成
    $smarty->assign('token', $firewall->set_token('research_add'));
...
elseif ($rec == 'insert') {
   ...
   if(!empty($_POST['file'])){
    ...
    // 继承栏目的密码属性
	$is_password = $dou->get_one("SELECT is_password FROM " . $dou->table('research_category') . " Where cat_id = '{$_POST['cat_id']}' ");
	$password = $dou->get_one("SELECT password FROM " . $dou->table('research_category') . " Where cat_id = '{$_POST['cat_id']}' ");
    ...
    // CSRF防御令牌验证
    $firewall->check_token($_POST['token'], 'research_add');
    ...
    $dou->create_admin_log('课题研究文章添加' . ': ' . $_POST['title']);
    $dou->dou_msg('课题研究文章添加成功', $_POST['cat_id']?'special_page.php?cat_id='.$_POST['cat_id']:'special_page.php');
} 
 ...
 */
elseif ($rec == 'edit') {
    $smarty->assign('ur_here', '课题研究文章编辑');
    ...
    $href = $article['cat_id']?'special_page.php?cat_id='.$article['cat_id']:'special_page.php';
    $smarty->assign('action_link', array (
            'text' => $_LANG['article'],
            'href' => $href, 
    ));
    // CSRF防御令牌生成
    $smarty->assign('token', $firewall->set_token('research_edit'));
...
//        $litpic = GetDDImage('none',$picname,$ddisremote);
   ...
   if(empty($up_file) && !empty($_POST['file'])){
    ...
    // CSRF防御令牌验证
    $firewall->check_token($_POST['token'], 'research_edit');
    $_POST['micromial'] =  $_POST['micromial'] ? $_POST['micromial'] : 0;
    $_POST['ctype'] =  $_POST['ctype'] ? $_POST['ctype'] : 0;
    $add_time = !empty($_POST['add_time'])? strtotime($_POST['add_time']) : time();
    $sql = "UPDATE " . $dou->table('research') . " SET cat_id = '$_POST[cat_id]', title = '$_POST[title]', defined = '$_POST[defined]' ,content = '$body'" . $up_file . ", keywords = '$keywords', description = '$description',source='$_POST[source]',micromail='$_POST[micromail]',custom_url = '$_POST[custom_url]',add_time='$add_time',ctype='$_POST[ctype]',password = '$_POST[password]',homeshow='$_POST[homeshow]' WHERE id = '$_POST[id]' ";

    $dou->query($sql);
    
    $dou->create_admin_log('课题研究文章编辑' . ': ' . $_POST['title']);
    $dou->dou_msg('课题研究文章编辑成功', $_POST['cat_id']?'special_page.php?cat_id='.$_POST['cat_id']:'special_page.php');
} 
...
elseif ($rec == 'set_sort') {
    // 验证并获取合法的ID
    $id = $check->is_number($_REQUEST['id']) ? $_REQUEST['id'] : $dou->dou_msg($_LANG['illegal'], 'special_page.php');
...
elseif ($rec == 'del') {
    ...
    if (isset($_POST['confirm']) ? $_POST['confirm'] : '') {
		...
		if(!check_aticle_is_self($id)){
			 ...
			 $dou->dou_msg("无权限删除", 'special_page.php?cat_id='.$cat_id, '', '3');
		}
        ...
        $dou->delete($dou->table('research'), "id = $id", "special_page.php?cat_id=$cat_id");
    } else {
        $_LANG['del_check'] = preg_replace('/d%/Ums', $title, $_LANG['del_check']);
        $dou->dou_msg($_LANG['del_check'], 'special_page.php', '', '30', "special_page.php?rec=del&id=$id");
    }
} 
        ...
        if ($_POST['action'] == 'del_all') {
			...
			foreach($_POST['checkbox'] as $k=>$e){
			 ...
			 if (is_array($_POST['checkbox']) && !empty($_POST['checkbox']) ) {
				 $cat_id = $dou->get_one("SELECT cat_id FROM " . $dou->table('research') . " WHERE id = '{$_POST['checkbox'][0]}'");
				 $dou->del_all('research', $_POST['checkbox'], 'id', 'image','special_page.php?cat_id='.$cat_id);
			 }else{
        	...
        	$dou->auditing_all('research', $_POST['checkbox'], 'id');
        } else {
...
elseif ($rec == 'auditing') {
	...
	$href = isset($_GET['cat_id']) && intval($_GET['cat_id']) > 0 ? 'special_page.php?rec=add&cat_id='.intval($_GET['cat_id']):'special_page.php?rec=add';
	$smarty->assign('action_link', array (
	...
	// 筛选条件
	if($_USER['group_id'] > 2){
		$where = ' WHERE cat_id IN (' . $cat_id . $dou->dou_child_id('research_category', $cat_id) . ') And stype = 1 And auditing = 0 And cat_id not in ('.$backup_group_id.')';
	}else{
		$where = ' WHERE cat_id IN (' . $cat_id . $dou->dou_child_id('research_category', $cat_id) . ') And stype = 1 And auditing = 0';
	}
	...
	$sql = "SELECT id, title, cat_id, image, add_time,auditing FROM " . $dou->table('research') . $where . " ORDER BY id DESC" . $limit;
	/* echo $sql;
	exit; */
	$query = $dou->query($sql);
	while ($row = $dou->fetch_array($query)) {
...
function get_sort_article() {
    ...
    $sql = "SELECT id, title FROM " . $GLOBALS['dou']->table('research') . " WHERE stype=1 and sort > 0 ORDER BY sort DESC" . $limit;
...
function check_aticle_is_self($aid){
		...
		if($GLOBALS['dou']->get_one("SELECT id FROM " . $GLOBALS['dou']->table('research') . " WHERE id = '$aid' and user_id= '{$GLOBALS['_USER']['user_id']}' ")){
			return true;
		}
		return false;
	}
