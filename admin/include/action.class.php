...
class Action extends Common {
     ...
     * +----------------------------------------------------------
     */
    function del_all($module, $checkbox, $field_filter, $field_image = '',$back_url) {
        $sql_in = $this->create_sql_in($_POST['checkbox']);
		...
		if(!empty($back_url)){
			$this->dou_msg($GLOBALS['_LANG']['del_succes'],$back_url);
		}else{
     ...
     * +----------------------------------------------------------
     */
    function auditing_all($module, $checkbox, $field_filter) {
    	$sql_in = $this->create_sql_in($_POST['checkbox']);
    	// 从数据库中更新所选内容
    	$sql = "Update " . $this->table($module) . "Set auditing = '1'  WHERE " . $field_filter . " " . $sql_in;
//     	echo $sql;
//     	exit;
    	$this->query($sql);
        
    	$this->create_admin_log("审核文章" . ': ' . strtoupper($module) . ' ' . addslashes($sql_in));
    	$this->dou_msg('审核成功', $module . '.php?rec=auditing');
    }
    /**
     * +----------------------------------------------------------
     * 删除图片
