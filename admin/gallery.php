...
if ($rec == 'active') {
	...
	$sql = "UPDATE ".$dou->table('gallery')." Set is_show = '{$val}' Where id ='{$id}' ";
	$dou->query($sql);
	echo 'success';
}
if ($rec == 'default') {
    ...
    // 获取参数
    $cat_id = $check->is_number($_REQUEST['gcat_id']) ? $_REQUEST['gcat_id'] : 0;
    ...
    $sql = "SELECT id, title, cat_id, image, add_time,is_show,user_id FROM " . $dou->table('gallery') . $where . " ORDER BY id DESC" . $limit;
...
elseif ($rec == 'insert') {
    ...
    $sql = "INSERT INTO " . $dou->table('gallery') . " (id, cat_id, title, image, gallery ,keywords, add_time, description,user_id)" . " VALUES (NULL, '$_POST[cat_id]', '$_POST[title]', '$file', '$gallery', '$_POST[keywords]', '$add_time', '$_POST[description]','$_USER[user_id]')";
    $dou->query($sql);
...
elseif ($rec == 'edit') {
    ...
    $smarty->assign('IS_SELF', $gallery['user_id'] == $_USER['user_id']);
    $smarty->display('gallery.htm');
} 
...
elseif ($rec == 'del') {
    ...
    if (isset($_POST['confirm']) ? $_POST['confirm'] : '') {
        // 删除相应商品图片
		if(!check_gallery_is_self($id)){
			
			 $dou->dou_msg("无权限删除", 'gallery.php?cat_id=74', '', '3');
		}
    ...
    if (is_array($_POST['checkbox'])) {
        if ($_POST['action'] == 'del_all') {
			foreach($_POST['checkbox'] as $k=>$e){
				if(!check_gallery_is_self($e)){
					unset($_POST['checkbox'][$k]);
				}
			}
			 if (is_array($_POST['checkbox']) && !empty($_POST['checkbox']) ) {
				 // 批量图片删除
				$dou->del_all('gallery', $_POST['checkbox'], 'id', 'image','gallery.php?cat_id=74');
			 }else{
				 $dou->dou_msg("无权删除");
			 }
...
//     var_dump($_FILES);
//     exit;
    $img->upload_gallery('gallery_file', $images_dir, $action_id); // 文件上传
} 
...
function get_gallery_list($id = '') {
    if ($id) {
        ...
        $images = $gallery ? unserialize($gallery) : glob(ROOT_PATH . $images_dir . $id . '_*.*');
    } else {
...
function check_gallery_is_self($aid){
		if(defined('IS_ADMIN_NEW')){
			return true;
		}
		
		if($GLOBALS['dou']->get_one("SELECT id FROM " . $GLOBALS['dou']->table('gallery') . " WHERE id = '$aid' and user_id= '{$GLOBALS['_USER']['user_id']}' ")){
			return true;
		}
		return false;
}
