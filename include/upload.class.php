...
class Upload {
     ...
     * +----------------------------------------------------------
     */
    function Upload($images_dir = '../upload/', $thumb_dir = 'thumb/', $upfile_type = 'jpg,gif,png', $upfile_size_max = '512000') {
    ...
    function upload_gallery($upfile, $images_url, $image_name = '') {
        ...
        if(is_array($_FILES[$upfile]["name"])){
        	...
        	for($i=0; $i < $count; $i++ ){
        		$name = explode(".", $_FILES[$upfile]["name"][$i]); // 将上传前的文件以“.”分开取得文件类型
        		// var_dump($_FILES[$upfile]["name"][$i]);
        		$img_count = count($name); // 获得截取的数量
        		$img_type = $name[$img_count - 1]; // 取得文件的类型
        		if (stripos($this->upfile_type, $img_type) === false) {
        		...
        		$file_name = $image_name . '_' . $rand;
        		$photo = $file_name . "." . $img_type; // 写入数据库的文件名
        		//var_dump($photo);
        		$upfile_name = $this->images_dir . $photo; // 上传后的文件名称
        		$upfile_ok = move_uploaded_file($_FILES[$upfile]["tmp_name"][$i], $upfile_name);
        		if ($upfile_ok) {
        			$img_size = $_FILES[$upfile]["size"][$i];
        			$img_size_kb = round($img_size / 1024);
        			if ($img_size_kb > $this->upfile_size_max) {
        				@unlink($upfile_name);
