...
class Firewall {
	 ...
	 * +----------------------------------------------------------
	 */
	function get_legal_user($module, $id = '') {
		// 如果有设置则验证合法性，验证通过的情况包括为空和赋值合法，分类页允许ID为空，详细页（包括单页面）不允许ID为空
		if ((isset ( $id ) && ! $GLOBALS ['check']->is_number ( $id )))
			return - 1;
		if (strstr ( $module, 'admin' )) {
			$get_id = $GLOBALS ['dou']->query ( "SELECT * FROM " . $GLOBALS ['dou']->table ( $module ) . " WHERE user_id = '$id'" );
			$get_id = $GLOBALS ['dou']->fetch_array($get_id);
		}elseif(strstr ( $module, 'blog_category' )){
			$get_id = $GLOBALS ['dou']->get_one ( "SELECT blog_id FROM " . $GLOBALS ['dou']->table ( $module ) . " WHERE blog_id = '$id'" );
		}elseif(strstr ( $module, 'article' )){
			$get_id = $GLOBALS ['dou']->get_one ( "SELECT id FROM " . $GLOBALS ['dou']->table ( $module ) . " WHERE id = '$id'" );
		}
		
		
		$legal = isset ( $get_id ) && ! empty ( $get_id ) ? $get_id : - 1;
