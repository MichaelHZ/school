...
class Common extends DbMysql
{
     ...
     * +----------------------------------------------------------
     */
    function rewrite_special_url($module, $articleId,$value = '')
    {
        ...
        if ($GLOBALS ['_CFG'] ['rewrite']) {
            ...
            $special = $id ? '&sid='.$id : '';
            $req = $rec ? '?rec=' . $rec : ($articleId ? '?id=' . $articleId .$special  : '');
            $url = $module . '.php' . $req;
        }
    ...
    {
        $data = $this->fetch_array_all($this->table($table), 'sort ASC'," where template = 'default'");
     ...
     * +----------------------------------------------------------
     */
    function dou_child_id_special($table, $parent_id = '0', &$child_id = '')
    {
     ...
     * +----------------------------------------------------------
     */
    function get_show_list($type = 'pc',$sortStart=0,$sortEnd=0)
    {
        if ($type) {
            $where = " WHERE type = '$type'";
        }

        if(!empty($sortStart) && !empty($sortEnd)){
            $sql = "SELECT * FROM " . $this->table('show') . $where . " And sort >= ".$sortStart." And sort <= '".$sortEnd."' ORDER BY sort ASC, id ASC";
            //echo $sql;
        }else{
    ...
    {
        $category = array();
        //var_dump($GLOBALS['_USER']);
        if ($GLOBALS['_USER']['user_id'] > 0) {
            $data = $this->fetch_array_all($this->table($table), 'sort ASC');
        } else {
            $data = $this->fetch_array_all($this->table($table), 'sort ASC', ' WHERE atype = 0 ');
        }
     ...
     * +----------------------------------------------------------
     */
    function pager($table, $page_size = 10, $page, $page_url = '', $where = '', $get = '', $close_rewrite = false, $cur = 9)
    {
...
// 		exit;
        ...
        // 页码
        $cur_limit = ($page % $cur == 0) ? 9 : ($page % $cur);

        for ($i = 0; $i < $page_count; $i++) {
            $tmp['index'] = $i + 1;
            $index = $i + 1;
            if (($index > ($page - $cur_limit)) && ($index <= ($page + $cur - $cur_limit))) {
                ...
                $tmp['index'] = $index;
                $tmp['url'] = $page_url . $get_page . $tmp['index'] . $get;
                $cur_pager[] = $tmp;
            }
        }
...
//         exit;
     ...
     * +----------------------------------------------------------
     */
    function special_pager($table, $page_size = 10, $page, $page_url = '', $where = '', $get = '', $close_rewrite = false, $cur = 9)
    {
        $sql = "SELECT * FROM " . $this->table($table) . $where;
...
//         exit;
     ...
     * +----------------------------------------------------------
     */
    function blog_pager($table, $page_size = 10, $page, $page_url = '', $where = '', $get = '', $close_rewrite = false, $cur = 'pager')
    {
        $sql = "SELECT * FROM " . $this->table($table) . $where;
// 		echo $sql;
// 		exit;
        ...
        $start = ($page - 1) * $page_size;
        $limit = " LIMIT $start, $page_size";
         //var_dump($cur_pager);exit;
        $GLOBALS ['smarty']->assign($cur, $pager);
     ...
     * +----------------------------------------------------------
     */
    function special_lift($module, $id, $cat_id)
    {
        $field = $this->field_exist($module, 'title') ? 'title' : 'name'; // 判断包含title字段还是name字段

        //$cid = $this->get_one("select cat_id from ".$this->table($module)." where id =".$id);
        ...
        if ($lift ['previous'])
            $lift ['previous'] ['url'] = $this->rewrite_special_url('special',$cat_id,  $lift ['previous'] ['id']);
        // 下一项
        $lift ['next'] = $this->fetch_assoc($this->query("SELECT id, " . $field . " FROM " . $this->table($module) . " WHERE id < $id" . $screen . " ORDER BY id DESC"));
        if ($lift ['next'])
            $lift ['next'] ['url'] = $this->rewrite_special_url('special',$cat_id, $lift ['next'] ['id']);
     ...
     * +----------------------------------------------------------
     */
    function blog_lift($module, $id, $blog_id)
    {
        $field = $this->field_exist($module, 'title') ? 'title' : 'name'; // 判断包含title字段还是name字段
        $screen = $blog_id ? " AND user_id = '$blog_id' and stype='2' " : ''; // 判断是否有分类筛选

        // 上一项
        $lift ['previous'] = $this->fetch_assoc($this->query("SELECT id, " . $field . " FROM " . $this->table($module) . " WHERE id > $id" . $screen . " ORDER BY id ASC"));
        if ($lift ['previous'])
            $lift ['previous'] ['url'] = $this->rewrite_url('blog', $lift ['previous'] ['id']) . '&blog_id=' . $blog_id;
        // 下一项
        $lift ['next'] = $this->fetch_assoc($this->query("SELECT id, " . $field . " FROM " . $this->table($module) . " WHERE id < $id" . $screen . " ORDER BY id DESC"));
        if ($lift ['next'])
            $lift ['next'] ['url'] = $this->rewrite_url('blog', $lift ['next'] ['id']) . '&blog_id=' . $blog_id;;
    ...
    // 查找上三级栏目
    function get_top_three_cats($cat_id,$level = 2){
        ...
        $cats = array();
        while(1){
            $cat_info = $this->get_row("SELECT parent_id,cat_id,cat_name FROM " . $this->table('article_category') . " Where cat_id = '" . $cur_id . "'");

            if(empty($cat_info)){
                break;
            }
            array_push($cats,$cat_info);

            if($cat_info['parent_id'] == 0){
                break;
            }

            $cur_id = $cat_info['parent_id'];
        }
    ...
    function get_two_catid($cat_id)
    {
        ...
        if ($cat_id > 0) {
            ...
            $index = 0;
            $cur_id = $cat_id;
            if (!($parent_id = $this->get_one("SELECT parent_id FROM " . $this->table('article_category') . " Where cat_id = '" . $cur_id . "'"))) {
                // 根栏目
                $rt['root'] = $cur_id;
                ...
                if ($child_id = $this->get_one("SELECT cat_id FROM " . $this->table('article_category') . " Where parent_id = '" . $cur_id . "' order by sort")) {
                    // 二级栏目
                    $rt['root'] = $this->get_one("SELECT parent_id FROM " . $this->table('article_category') . " Where cat_id = '" . $cur_id . "'");
                    $rt['two'] = $cur_id;
                    $rt ['child'] = $child_id;
                } else {
                    // 三级栏目
                    $rt ['child'] = $cur_id;
                    $rt['two'] = $this->get_one("SELECT parent_id FROM " . $this->table('article_category') . " Where cat_id = '" . $cur_id . "'");
                    $root = $this->get_one("SELECT parent_id FROM " . $this->table('article_category') . " Where cat_id = '" . $rt['two'] . "'");
                    if (!empty($root)) {
                        $rt ['root'] = $root;
                    } else {
                        $rt ['root'] = $rt['two'];
    ...
    // 获取第二层栏目的id
    function get_two_catid_old($cat_id)
    {
        ...
        if ($cat_id > 0) {
            ...
            if (!($parent_id = $this->get_one("SELECT parent_id FROM " . $this->table('article_category') . " Where cat_id = '" . $cur_id . "'"))) {
                ...
                $rt['two'] = $this->get_one("SELECT cat_id FROM " . $this->table('article_category') . " Where parent_id = '" . $cur_id . "' order by sort");
                $child = $this->get_one("SELECT cat_id FROM " . $this->table('article_category') . " Where parent_id = '" . $rt['two'] . "' order by sort");
                $rt['child'] = $child ? $child : $rt['two'];
     ...
     * +----------------------------------------------------------
     */
    function get_home_position($type = 'pc_home'){
        $data = array();
        $position = $this->fetch_array_all($this->table('home'),'id asc '," Where type = '".$type."'");
