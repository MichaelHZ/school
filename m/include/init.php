...
if(preg_match('/index\.php/i',$_SERVER['URL'])){
	$cur = 1;
}elseif(preg_match('/news_category\.php/i',$_SERVER['URL'])){
	$cur = 2;
}elseif(preg_match('/article_category\.php\?id\=4/i',$_SERVER['REQUEST_URI'])){
	$cur = 3;
}elseif(preg_match('/guestbook\.php/i',$_SERVER['URL'])){
	$cur = 4;
}
//var_dump($cur);
$smarty->assign("_current", $cur);
// 开启缓冲区
ob_start();
?>