...
function listTwo($categoryTable,$articleTable){
    global $smarty,$_REQUEST,$dou,$cat_id,$check,$_DISPLAY;

    $where = ' WHERE cat_id = '."'".$_REQUEST['sid']."'";

    $cat_name = $dou->get_one("SELECT cat_name FROM " . $dou->table($categoryTable) . $where);

    $where = ' WHERE cat_id = '."'".$_REQUEST['cid']."'";

    $cat_name_sub = $dou->get_one("SELECT cat_name FROM " . $dou->table($categoryTable) . $where);
    ...
    $where = ' WHERE cat_id IN (' . $_REQUEST['cid'] . $dou->dou_child_id('article_category',  $_REQUEST['cid']) . ") And auditing = '1' And micromail = '1' ";
    $page = $check->is_number($_REQUEST['page']) ? trim($_REQUEST['page']) : 1;

    $limit = $dou->blog_pager($articleTable, ($_DISPLAY['article'] ? $_DISPLAY['article'] : 10), $page, $dou->rewrite_special_url('special_list_two', $_REQUEST['cid']), $where, $get = '', $close_rewrite = false);

    $sql = "SELECT * FROM " . $dou->table($categoryTable) . $where . " ORDER BY sort  asc" . $limit;
    $query = $dou->query($sql);

    $smarty->assign('article_list_sid',get_special_article_list($categoryTable,$articleTable,$cat_id,$_REQUEST['sid'],$_REQUEST['cid'],$limit));
...
function get_special_article_list($categoryTable,$articleTable,$cat_id,$special_id,$cid,$limit)
{
    global $dou;
    ...
    if ($special_id == -1) {
        ...
        $where = ' WHERE cat_id IN (' . $cid . $dou->dou_child_id_special($categoryTable, $cid) . ')'  ;
    }
    /* 获取文章列表 */
    $sql = "SELECT id, title, content, image, cat_id, add_time, click, description,ctype,keywords,custom_url FROM " . $dou->table($articleTable) . $where . " ORDER BY add_time  DESC" .$limit;
