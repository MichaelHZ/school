...
if(!empty($_REQUEST['sid'])){
    $smarty->display($child['template'].'_sub.dwt');
}else{
...
function listOne($categoryTable){
    ...
    while ($row = $dou->fetch_array($query)) {
        ...
        $url = $dou->rewrite_special_url('special_list_one',$cat_id, $row ['cat_id']);
...
function listTwo($categoryTable,$articleTable){
    ...
    $cat_name = $dou->get_row("SELECT cat_name,description FROM " . $dou->table($categoryTable) . $where);
    ...
    $cat_list = array(
        'title' =>$cat_name['cat_name'],
        'description' => $cat_name['description'],
    ...
    $page = $check->is_number($_REQUEST['page']) ? trim($_REQUEST['page']) : 1;
//var_dump($dou->rewrite_special_url('special_list_two',  $cat_id,$_REQUEST['sid']).'&cid='.$_REQUEST['cid']);exit;
    $dou->blog_pager($articleTable, ($_DISPLAY['article'] ? $_DISPLAY['article'] : 10), $page, $dou->rewrite_special_url('special_list_two',$cat_id, $_REQUEST['sid']).'&cid='.$_REQUEST['cid'], $where, $get = '', $close_rewrite = false);
